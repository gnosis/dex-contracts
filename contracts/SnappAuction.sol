...
contract SnappAuction is SnappBase {
    ...
    event StandingSellOrderBatch(uint256 currentBatchIndex, uint256 validFromAuctionId, uint16 accountId, bytes packedOrders);

    event AuctionSettlement(uint256 auctionId, uint256 stateIndex, bytes32 stateHash, bytes pricesAndVolumes);

    event AuctionInitialization(uint16 maxOrders, uint16 numReservedAccounts, uint16 ordersPerReservedAccount);

    constructor() public {
        emit AuctionInitialization(AUCTION_BATCH_SIZE, AUCTION_RESERVED_ACCOUNTS, AUCTION_RESERVED_ACCOUNT_BATCH_SIZE);
    }

    /**
     * Public View Methods
     */
    function getAuctionCreationTimestamp(uint256 slot) public view returns (uint256) {
    ...
    function getStandingOrderValidFrom(uint16 userId, uint128 batchIndex) public view returns (uint256) {
        return standingOrders[userId].reservedAccountOrders[batchIndex].validFromIndex;
    }

    function getStandingOrderValidTo(uint16 userId, uint128 batchIndex) public view returns (uint256) {
        uint256 validTo = standingOrders[userId].reservedAccountOrders[batchIndex + 1].validFromIndex;
        if (validTo == 0) {
            return MAX_UINT;
        } else {
            return validTo - 1;
    ...
    function getStandingOrderCounter(uint16 userId) public view returns (uint256) {
     ...
     */
    function placeStandingSellOrder(bytes memory packedOrders) public onlyRegistered() {
        ...
        if (auctionIndex == MAX_UINT || block.timestamp > (auctions[auctionIndex].creationTimestamp + 3 minutes)) {
        ...
        for (uint256 i = 0; i < numOrders; i++) {
            ...
            orderHash = sha256(abi.encodePacked(orderHash, encodeOrder(accountId, buyToken, sellToken, buyAmount, sellAmount)));
        }
    ...
    function placeSellOrder(uint8 buyToken, uint8 sellToken, uint96 buyAmount, uint96 sellAmount) public {
    ...
    function auctionSolutionBid(
        ...
        require(slot == 0 || auctions[slot - 1].appliedAccountStateIndex != 0, "Previous auction not yet resolved!");
        ...
        require(block.timestamp < biddingStartTime(slot) + 3 minutes, "Bidding period for this auction has expired");
    ...
    function applyAuction(
        ...
        require(block.timestamp > biddingStartTime(slot) + 3 minutes, "Requested auction still in bidding phase or earlier");

        if (block.timestamp < biddingStartTime(slot) + 270 seconds && auctions[slot].solver != address(0)) {
            // Winner Apply Auction
            require(auctions[slot].solver == msg.sender, "Only winner of bidding phase may apply auction here");
    ...
    function calculateOrderHash(uint256 slot, uint128[] memory _standingOrderIndex) public view returns (bytes32) {
    ...
    function orderBatchIsValidAtAuctionIndex(uint256 _auctionIndex, uint8 userId, uint128 orderBatchIndex)
        public
        view
        returns (bool)
    {
    ...
    function encodeOrder(uint16 accountId, uint8 buyToken, uint8 sellToken, uint96 buyAmount, uint96 sellAmount)
        internal
        view
        returns (bytes32)
    {
    ...
    function internalApplyAuction(uint256 slot, bytes32 newStateRoot, bytes memory pricesAndVolumes) internal {
