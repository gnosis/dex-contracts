...
import { decodeOrders } from "../src/encoding";
...
contract("BatchExchange", async (accounts) => {
  ...
  before(async () => {
    ...
    BATCH_TIME = (await batchExchange.BATCH_TIME()).toNumber();
  });
  ...
  describe("addToken()", () => {
    it("feeToken is set by default", async () => {
      ...
      assert.equal(
        (await batchExchange.tokenAddressToIdMap(feeToken.address)).toNumber(),
        0,
      );
      assert.equal(
        await batchExchange.tokenIdToAddressMap(0),
        feeToken.address,
      );
    });
    it("Anyone can add tokens", async () => {
      ...
      assert.equal(
        (await batchExchange.tokenAddressToIdMap(token_1.address)).toNumber(),
        1,
      );
      assert.equal(await batchExchange.tokenIdToAddressMap(1), token_1.address);
      const token_2 = await ERC20.new();
      await batchExchange.addToken(token_2.address, { from: user_2 });

      assert.equal(
        (await batchExchange.tokenAddressToIdMap(token_2.address)).toNumber(),
        2,
      );
      assert.equal(await batchExchange.tokenIdToAddressMap(2), token_2.address);
    });
    it("Rejects same token added twice", async () => {
      ...
      await truffleAssert.reverts(
        batchExchange.addToken(token.address),
    ...
    it("No exceed max tokens", async () => {
      ...
      await truffleAssert.reverts(
        batchExchange.addToken((await ERC20.new()).address),
        "Max tokens reached",
      );
    });
    it("Burns 10 OWL when adding token", async () => {
      ...
      assert(
        owlAmount.eq(await owlProxy.allowance(user_1, batchExchange.address)),
      );
    ...
    it("throws if OWL is not burned", async () => {
      ...
      await truffleAssert.reverts(
        batchExchange.addToken(token.address, { from: user_1 }),
      );
    });
  });
  describe("placeOrder()", () => {
    it("rejects orders between same tokens", async () => {
      ...
      await truffleAssert.reverts(
        batchExchange.placeValidFromOrders.call(
          [0],
          [0],
          [currentBatch - 1],
          [1],
    ...
    it("places order and verifys contract storage is updated correctly", async () => {
      ...
      const orderResult = toOrderResult(await batchExchange.orders(user_1, id));
      assert.equal(
        orderResult.priceDenominator.toNumber(),
        20,
        "priceDenominator was stored incorrectly",
      );
      assert.equal(
        orderResult.priceNumerator.toNumber(),
        10,
        "priceNumerator was stored incorrectly",
      );
      assert.equal(
        orderResult.sellToken.toNumber(),
        1,
        "sellToken was stored incorrectly",
      );
      assert.equal(
        orderResult.buyToken.toNumber(),
        0,
        "buyToken was stored incorrectly",
      );
      assert.equal(
        orderResult.validFrom.toNumber(),
        currentStateIndex.toNumber(),
        "validFrom was stored incorrectly",
      );
      assert.equal(
        orderResult.validUntil.toNumber(),
        3,
  ...
  describe("placeValidFromOrders()", () => {
    ...
    it("places single order with specified validFrom", async () => {
      const batchExchange = await setupGenericStableX();
      const currentBatch = (await batchExchange.getCurrentBatchId()).toNumber();
      const id = await batchExchange.placeValidFromOrders.call(
        [0],
        [1],
        [currentBatch],
        [3],
      ...
      const orderResult = toOrderResult(
        await batchExchange.orders(user_1, id.toString()),
      );
      assert.equal(
        orderResult.priceDenominator.toNumber(),
        20,
    ...
    it("places multiple orders with sepcified validFrom", async () => {
      ...
      const orderResult0 = toOrderResult(await batchExchange.orders(user_1, 0));
      assert.equal(
        orderResult0.priceDenominator.toNumber(),
        20,
        "order 1: priceDenominator was stored incorrectly",
      );
      assert.equal(
        orderResult0.priceNumerator.toNumber(),
        10,
        "order 1: priceNumerator was stored incorrectly",
      );
      assert.equal(
        orderResult0.sellToken.toNumber(),
        1,
        "order 1: sellToken was stored incorrectly",
      );
      assert.equal(
        orderResult0.buyToken.toNumber(),
        0,
        "order 1: buyToken was stored incorrectly",
      );
      // Note that this order will be stored, but never valid. However, this can not affect the exchange in any maliciouis way!
      assert.equal(
        orderResult0.validFrom.toNumber(),
        currentBatch,
        "order 1: validFrom was stored incorrectly",
      );
      assert.equal(
        orderResult0.validUntil.toNumber(),
        3,
        "order 1: validUntil was stored incorrectly",
      );

      const orderResult1 = toOrderResult(await batchExchange.orders(user_1, 1));
      assert.equal(
        orderResult1.priceDenominator.toNumber(),
        21,
        "order 2: priceDenominator was stored incorrectly",
      );
      assert.equal(
        orderResult1.priceNumerator.toNumber(),
        11,
        "order 2: priceNumerator was stored incorrectly",
      );
      assert.equal(
        orderResult1.sellToken.toNumber(),
        0,
        "order 2: sellToken was stored incorrectly",
      );
      assert.equal(
        orderResult1.buyToken.toNumber(),
        1,
        "order 2: buyToken was stored incorrectly",
      );
      // Note that this order will be stored, but never valid. However, this can not affect the exchange in any maliciouis way!
      assert.equal(
        orderResult1.validFrom.toNumber(),
        currentBatch,
        "order 2: validFrom was stored incorrectly",
      );
      assert.equal(
        orderResult1.validUntil.toNumber(),
        4,
        "order 2: validUntil was stored incorrectly",
      );
    });
  });
  describe("cancelOrders()", () => {
    it("invalidates valid order as of next batch", async () => {
      ...
      const currentStateIndex = (
        await batchExchange.getCurrentBatchId()
      ).toNumber();
      ...
      assert.equal(
        toOrderResult(
          await batchExchange.orders(user_1, id),
        ).validUntil.toNumber(),
        currentStateIndex,
        "validUntil was stored incorrectly",
      );
    ...
    it("frees storage of orders that are not yet valid", async () => {
      ...
      assert.equal(
        toOrderResult(
          await batchExchange.orders(user_1, 0),
        ).priceDenominator.toNumber(),
        0,
        "Order data was not cleared",
      );
    });
  });
  describe("replaceOrders()", () => {
    it("cancels and creates new orders", async () => {
      ...
      await batchExchange.placeOrder(0, 1, 3, 10, 20, { from: user_1 });
      const order2 = await sendTxAndGetReturnValue(
      ...
      assert.equal(
        ...
        1,
        "First order should be present",
      );
      assert.equal(
        toOrderResult(
          await batchExchange.orders(user_1, 1),
        ).sellToken.toNumber(),
        0,
      ...
      assert.equal(
        toOrderResult(
          await batchExchange.orders(user_1, 2),
        ).sellToken.toNumber(),
        6,
        "Third order should be present",
      );
      assert.equal(
        toOrderResult(
          await batchExchange.orders(user_1, 3),
        ).sellToken.toNumber(),
        7,
        "Fourth order should be present",
      );
    });
  });
  describe("submitSolution()", () => {
    ...
    it("[Basic Trade] places two orders and returns calculated utility", async () => {
      ...
      assert(
        objectiveValue.gtn(0),
        "the computed objective value is greater than 0",
      );
    ...
    it("rejects acclaimed marginally improved solutions", async () => {
      ...
      const objectiveValue = await batchExchange.getCurrentObjectiveValue();
      const improvementDenominator = await batchExchange.IMPROVEMENT_DENOMINATOR();

      const tooLowNewObjective = objectiveValue
        .mul(improvementDenominator.addn(1))
        .div(improvementDenominator);
    ...
    it("rejects marginally better solutions", async () => {
      ...
      const firstSolution = solutionSubmissionParams(
        tradeCase.solutions[0],
        accounts,
        orderIds,
      );
      await batchExchange.submitSolution(
      ...
      const insufficientlyBetterSolution = solutionSubmissionParams(
        tradeCase.solutions[1],
        accounts,
        orderIds,
      );
      const improvementDenominator = await batchExchange.IMPROVEMENT_DENOMINATOR();
    ...
    it("[Basic Trade] places two orders and matches them in a solution with Utility > 0", async () => {
      const batchExchange = await setupGenericStableX();
      const feeToken = await batchExchange.tokenIdToAddressMap(0);
      const erc20_2 = await batchExchange.tokenIdToAddressMap(1);
      ...
      assert.equal(
        (await batchExchange.getBalance(user_1, feeToken)).toString(),
        basicTrade.deposits[0].amount
          .sub(getExecutedSellAmount(volume[0], prices[0], toETH(1)))
          .toString(),
        "Sold tokens were not adjusted correctly",
      );
      assert.equal(
        await (await batchExchange.getBalance(user_1, erc20_2)).toString(),
        volume[0].toString(),
        "Bought tokens were not adjusted correctly",
      );
      assert.equal(
        (await batchExchange.getBalance(user_2, erc20_2)).toString(),
        basicTrade.deposits[1].amount
          .sub(getExecutedSellAmount(volume[1], toETH(1), prices[0]))
          .toString(),
        "Sold tokens were not adjusted correctly",
      );
      assert.equal(
        await (await batchExchange.getBalance(user_2, feeToken)).toString(),
        volume[1].toString(),
        "Bought tokens were not adjusted correctly",
      );

      // This final assertion isn't really necessary here.
      const currentObjectiveValue = await batchExchange.getCurrentObjectiveValue();
      assert.equal(
        currentObjectiveValue.toString(),
    ...
    it("[Basic Trade] places two orders, matches them partially and then checks correct order adjustments", async () => {
      ...
      const partialSolution = solutionSubmissionParams(
      ...
      assert(
        await (await batchExchange.getBalance(user_1, erc20_2)).eq(volume[0]),
        "Bought tokens were not adjusted correctly",
      );
      assert(
        (await batchExchange.getBalance(user_2, erc20_2)).eq(
          basicTrade.deposits[1].amount.sub(
            getExecutedSellAmount(volume[1], toETH(1), prices[0]),
          ),
        ),
        "Sold tokens were not adjusted correctly",
      );
      assert(
        (await batchExchange.getBalance(user_2, feeToken)).eq(volume[1]),
        "Bought tokens were not adjusted correctly",
      );

      const orderResult1 = toOrderResult(
        await batchExchange.orders(user_1, orderIds[0]),
      );
      const orderResult2 = toOrderResult(
        await batchExchange.orders(user_2, orderIds[1]),
      );

      assert.equal(
        orderResult1.usedAmount.toString(),
      ...
      assert.equal(
        orderResult2.usedAmount.toString(),
    ...
    it("[Basic Trade] places two orders, first matches them partially and then fully in a 2nd solution submission", async () => {
      ...
      const partialObjectiveValue = await batchExchange.getCurrentObjectiveValue();
      assert.equal(
        partialObjectiveValue.toString(),
        partialSolution.objectiveValue.toString(),
      );
      ...
      assert.equal(
        (await batchExchange.getBalance(user_1, feeToken)).toString(),
        basicTrade.deposits[0].amount
          .sub(getExecutedSellAmount(partialBuyVolumes[0], prices[0], toETH(1)))
          .toString(),
        "Sold tokens were not adjusted correctly",
      );
      assert.equal(
      ...
      assert.equal(
        (await batchExchange.getBalance(user_2, erc20_2)).toString(),
        basicTrade.deposits[1].amount
          .sub(getExecutedSellAmount(partialBuyVolumes[1], toETH(1), prices[0]))
          .toString(),
        "Sold tokens were not adjusted correctly",
      );
      assert.equal(
      ...
      const fullSolution = solutionSubmissionParams(
        basicTrade.solutions[0],
        accounts,
        orderIds,
      );
      ...
      assert(
        (await batchExchange.getCurrentObjectiveValue()).eq(
          fullSolution.objectiveValue,
        ),
      );
      ...
      assert.equal(
        (await batchExchange.getBalance(user_1, feeToken)).toString(),
        basicTrade.deposits[0].amount
          .sub(getExecutedSellAmount(fullBuyVolumes[0], prices[0], toETH(1)))
          .toString(),
        "Sold tokens were not adjusted correctly",
      );
      assert.equal(
      ...
      assert.equal(
        (await batchExchange.getBalance(user_2, erc20_2)).toString(),
        basicTrade.deposits[1].amount
          .sub(getExecutedSellAmount(fullBuyVolumes[1], toETH(1), prices[0]))
          .toString(),
        "Sold tokens were not adjusted correctly",
      );
      assert.equal(
    ...
    it("ensures half of the token imbalance (fees) is burned and that better solutions don't double-burn", async () => {
      ...
      const nextBatchId = (await batchExchange.getCurrentBatchId()).toNumber();
      const secondorderIds = await placeOrders(
        batchExchange,
        accounts,
        secondTradeExample.orders,
        nextBatchId + 1,
      );
      await closeAuction(batchExchange);

      const initialFeeTokenBalance = await owlProxy.balanceOf(
        batchExchange.address,
      );
      const secondSolution = solutionSubmissionParams(
        secondTradeExample.solutions[0],
        accounts,
        secondorderIds,
      );
      ...
      const afterAuctionFeeTokenBalance = await owlProxy.balanceOf(
        batchExchange.address,
      );
      assert(
        initialFeeTokenBalance
          .sub(basicTrade.solutions[0].burntFees)
          .eq(afterAuctionFeeTokenBalance),
      );

      // Better second solution
      const betterSolution = solutionSubmissionParams(
        secondTradeExample.solutions[1],
        accounts,
        secondorderIds,
      );
      // This is where the first auction's fees should be burned!
      ...
      const afterBetterSolutionFeeBalance = await owlProxy.balanceOf(
        batchExchange.address,
      );
      assert(
        initialFeeTokenBalance
          .sub(basicTrade.solutions[0].burntFees)
          .eq(afterBetterSolutionFeeBalance),
      );
    });
    it("verifies the 2nd solution is correctly documented and can be reverted by a 3rd", async () => {
      ...
      assert(
        advancedTrade.solutions.length >= 3,
        "This test must always run on a sequence of at least three solutions.",
      );
      for (const solution of advancedTrade.solutions) {
        ...
        assert.equal(
          (await batchExchange.getBalance(user_1, feeToken)).toString(),
          advancedTrade.deposits[0].amount
            .sub(getExecutedSellAmount(volumes[0], prices[0], toETH(1)))
            .toString(),
          "Sold tokens were not adjusted correctly",
        );
        assert.equal(
          (await batchExchange.getBalance(user_1, erc20_2)).toString(),
          volumes[0].toString(),
          "Bought tokens were not adjusted correctly",
        );
        assert.equal(
          (await batchExchange.getBalance(user_2, erc20_2)).toString(),
          advancedTrade.deposits[1].amount
            .sub(getExecutedSellAmount(volumes[1], toETH(1), prices[0]))
            .toString(),
          "Sold tokens were not adjusted correctly",
        );
        assert.equal(
          (await batchExchange.getBalance(user_2, feeToken)).toString(),
          volumes[1].toString(),
          "Bought tokens were not adjusted correctly",
        );
      }
    ...
    it("[Basic Trade] rejects solution submission after 4 minute deadline is over", async () => {
      ...
      const time_remaining = (
        await batchExchange.getSecondsRemainingInBatch()
      ).toNumber();
    ...
    it("[Basic Trade] throws if order(s) not yet valid", async () => {
      ...
      for (const order of basicTrade.orders) {
        ...
        orderIds.push(
          ...
          )[0].toNumber(), // Because placeValidFromOrders returns a list of indices
        );
      }
      await closeAuction(batchExchange);
      const solution = solutionSubmissionParams(
    ...
    it("throws if sell volume is bigger than available balance", async () => {
      ...
      for (const deposit of basicTrade.deposits) {
        ...
        await batchExchange.deposit(
          tokenAddress,
          deposit.amount.sub(ERROR_EPSILON),
    ...
    it("grants fee surplus to solution submitter", async () => {
      ...
      assert.equal(
        ...
        await (await batchExchange.getBalance(solver, feeToken)).toString(),
    ...
    it("ensures fee deducted from previous submitter, when better solution is submitted", async () => {
      ...
      assert.equal(
        0,
        await (await batchExchange.getBalance(solver, feeToken)).toNumber(),
        "First submitter's reward was not reverted",
      );
    });
    it("ensures that a solution reversion can not be prevented by additional withdrawRequests", async () => {
      ...
      await batchExchange.requestWithdraw(
        await batchExchange.tokenIdToAddressMap(basicTrade.orders[1].buyToken),
        1,
        {
      ...
      await batchExchange.requestWithdraw(
        await batchExchange.tokenIdToAddressMap(basicTrade.orders[0].buyToken),
        1,
        {
    ...
    it("ensures credited tokens can't be withdrawn in same batch as solution submission", async () => {
      ...
      const buyToken = await batchExchange.tokenIdToAddressMap(
        basicTrade.orders[0].buyToken,
      );
      ...
      assert.equal(
        batchId + 1,
        (
          await batchExchange.lastCreditBatchId(relevantUser, buyToken)
        ).toNumber(),
    ...
    it("ensures credited feeToken reward can't be withdrawn in same batch as solution submission", async () => {
      ...
      assert.equal(
        batchId + 1,
        (await batchExchange.lastCreditBatchId(solver, feeToken)).toNumber(),
      );
      await truffleAssert.reverts(
    ...
    it("checks that the objective value is returned correctly after getting into a new batch", async () => {
      ...
      assert.equal(
        0,
        await (await batchExchange.getCurrentObjectiveValue()).toNumber(),
        "Objective value is not returned correct",
      );
    });
    ...
    it("reverts if max touched orders is exceeded", async () => {
      ...
      const batchId = (await batchExchange.getCurrentBatchId()).toNumber();
      const maxTouchedOrders = (
        await batchExchange.MAX_TOUCHED_ORDERS()
      ).toNumber();
    ...
    it("[Ring Trade] settles a ring trade between 3 tokens", async () => {
      ...
      for (let i = 0; i < basicRingTrade.orders.length; i++) {
        ...
        const sellToken = await batchExchange.tokenIdToAddressMap(
          order.sellToken,
        );
        const relevantUser = accounts[order.user];

        const sellTokenBalance = await batchExchange.getBalance(
          relevantUser,
          sellToken,
        );
        const buyTokenBalance = await batchExchange.getBalance(
          relevantUser,
          buyToken,
        );
    ...
    it("checks that currentPrices between different solutions are reset", async () => {
      ...
      const orderIds = await placeOrders(
        batchExchange,
        accounts,
        shortRingBetterTrade.orders,
        batchId + 1,
      );
      await closeAuction(batchExchange);

      const ringSolution = solutionSubmissionParams(
        shortRingBetterTrade.solutions[0],
        accounts,
        orderIds,
      );
      await batchExchange.submitSolution(
      ...
      assert.equal(
        ...
        (await batchExchange.currentPrices(2)).toString(),
        "CurrentPrice were not adjusted correctly",
      );

      const directSolution = solutionSubmissionParams(
        shortRingBetterTrade.solutions[1],
        accounts,
        orderIds,
      );
      await batchExchange.submitSolution(
      ...
      assert.equal(
        0,
        (await batchExchange.currentPrices(2)).toNumber(),
        "CurrentPrice were not adjusted correctly",
      );
    });
    it("checks that solution trades are deleted even if balances are temporarily negative while reverting ", async () => {
      ...
      const batchExchange = await setupGenericStableX();
      const feeToken = await batchExchange.tokenIdToAddressMap(0);
      const otherToken = await batchExchange.tokenIdToAddressMap(1);
      ...
      // User 0
      assert.equal(
        (await batchExchange.getBalance(accounts[0], otherToken)).toString(),
        solution.volumes[0].toString(),
        "Bought tokens were not adjusted correctly",
      );
      assert.equal(
        (await batchExchange.getBalance(accounts[0], feeToken)).toString(),
      ...
      assert.equal(
        0,
        (await batchExchange.getBalance(accounts[1], otherToken)).toNumber(),
        "Bought and sold tokens were not adjusted correctly",
      );
      assert.equal(
        0,
        (await batchExchange.getBalance(accounts[1], feeToken)).toNumber(),
        "Sold tokens were not adjusted correctly",
      );
      // User 2
      assert.equal(
        (await batchExchange.getBalance(accounts[2], feeToken)).toString(),
        solution.volumes[3].toString(),
        "Bought tokens were not adjusted correctly",
      );
      assert.equal(
        (await batchExchange.getBalance(accounts[2], otherToken)).toString(),
    ...
    it("ensures order's usedAmount is not updated, if tracking is unintended", async () => {
      ...
      await makeDeposits(
        batchExchange,
        accounts,
        exampleOrderWithUnlimitedAmount.deposits,
      );
      const batchId = (await batchExchange.getCurrentBatchId()).toNumber();
      const orderIds = await placeOrders(
        batchExchange,
        accounts,
        exampleOrderWithUnlimitedAmount.orders,
        batchId + 10,
      );
      await closeAuction(batchExchange);

      const firstSolution = solutionSubmissionParams(
        exampleOrderWithUnlimitedAmount.solutions[0],
        accounts,
        orderIds,
      );
      ...
      const orderResult1 = toOrderResult(
        await batchExchange.orders(
          firstSolution.owners[1],
          firstSolution.touchedorderIds[1],
        ),
      );
      assert.equal(
        orderResult1.usedAmount.toNumber(),
        0,
        "usedAmount was modified, although it should not have been modified",
      );
      ...
      const secondSolution = solutionSubmissionParams(
        exampleOrderWithUnlimitedAmount.solutions[1],
        accounts,
        orderIds,
      );
      await batchExchange.submitSolution(
      ...
      assert.equal(
        orderResult2.usedAmount.toNumber(),
        0,
        "usedAmount was modified, although it should not have been modified",
      );
      const orderResult3 = toOrderResult(
        await batchExchange.orders(
          secondSolution.owners[0],
          secondSolution.touchedorderIds[0],
        ),
      );
      assert.equal(
        orderResult3.usedAmount.toNumber(),
        0,
        "usedAmount was modified, although it should not have been modified",
      );
    });
    it("partially fills orders in one auction and then fills them some more in the next.", async () => {
      ...
      for (let i = 0; i < basicTrade.orders.length; i++) {
        ...
        const totalExecutedBuy = partialSolution.volumes[i].add(
          remainingBuyVolumes[i],
        );
  ...
  describe("getUsersPaginated()", async () => {
    const account_one_and_two = (accounts[0] + accounts[1].slice(2, 42))
      .toString()
      .toLowerCase();
    ...
    it("returns users when less than page size", async () => {
      ...
      assert.equal(
        ...
        (await batchExchange.getUsersPaginated(zero_address, 2)).toString(),
      );
    });
    ...
    it("returns first page and second page when larger than page size", async () => {
      ...
      assert.equal(
        accounts[2].toString().toLowerCase(),
        (await batchExchange.getUsersPaginated(accounts[1], 2)).toString(),
      );
    });
  });
  describe("getEncodedUserOrdersPaginated()", async () => {
    it("returns correct orders considering offset and pageSize", async () => {
      ...
      const auctionElements = decodeOrders(
        await batchExchange.getEncodedUserOrdersPaginated(user_1, 1, 1),
      );
      assert.equal(auctionElements[0].priceNumerator.toNumber(), 1);
    });
  });
  describe("getEncodedUserOrders()", async () => {
    it("returns null when there are no orders", async () => {
      const batchExchange = await setupGenericStableX();
      const auctionElements = await batchExchange.getEncodedUserOrders(
        accounts[0],
      );
    ...
    it("returns correct orders whether valid, cancelled or freed", async () => {
      ...
      const auctionElements = decodeOrders(
        await batchExchange.getEncodedUserOrders(user_1),
      );
      assert.equal(
        JSON.stringify(auctionElements),
        JSON.stringify([cancelledOrderInfo, freedOrderInfo, validOrderInfo]),
      );
    });
  });
  describe("getEncodedOrders()", async () => {
    it("returns all orders that are have ever been submitted", async () => {
      ...
      const auctionElements = decodeOrders(
        await batchExchange.getEncodedOrders(),
      );
      assert.equal(JSON.stringify(auctionElements), JSON.stringify(orderInfo));
    });
    it("credits balance when it's valid", async () => {
      const batchExchange = await setupGenericStableX(3);
      const erc20_1 = await batchExchange.tokenIdToAddressMap(1);
      const erc20_2 = await batchExchange.tokenIdToAddressMap(2);
      ...
      let auctionElements = decodeOrders(
        await batchExchange.getEncodedOrders(),
      );
      assert.equal(auctionElements[0].sellTokenBalance.toNumber(), 0);

      await waitForNSeconds(BATCH_TIME);

      auctionElements = decodeOrders(await batchExchange.getEncodedOrders());
      assert.equal(auctionElements[0].sellTokenBalance.toNumber(), 20);
    });
  ...
  describe("getEncodedUsersPaginated", async () => {
    it("returns empty bytes when no users", async () => {
      const batchExchange = await setupGenericStableX();
      const auctionElements = await batchExchange.getEncodedUsersPaginated(
        zero_address,
        0,
        10,
      );
    ...
    it("returns three orders one per page", async () => {
      ...
      const firstPage = decodeOrders(
        await batchExchange.getEncodedUsersPaginated(zero_address, 0, 1),
      );
      assert.equal(
      ...
      const secondPage = decodeOrders(
        await batchExchange.getEncodedUsersPaginated(user_1, 1, 1),
      );
      assert.equal(
      ...
      const thirdPage = decodeOrders(
        await batchExchange.getEncodedUsersPaginated(user_1, 2, 1),
      );
      assert.equal(
      ...
      // 4th page is empty
      assert.equal(
        await batchExchange.getEncodedUsersPaginated(user_2, 1, 1),
        null,
      );
    });
    it("returns three orders when page size is overlapping users", async () => {
      ...
      const page = decodeOrders(
        await batchExchange.getEncodedUsersPaginated(user_1, 1, 2),
      );
    ...
    it("returns three orders from three users with larger page size", async () => {
      ...
      const page = decodeOrders(
        await batchExchange.getEncodedUsersPaginated(zero_address, 0, 5),
      );
  ...
  describe("hasToken()", async () => {
    it("returns whether token was already added", async () => {
      ...
      assert.equal(await batchExchange.hasToken(erc20_1.address), false);
      await batchExchange.addToken(erc20_1.address);

      assert.equal(await batchExchange.hasToken(erc20_1.address), true);
    });
  });
});
...
function toOrderResult(result: [BN, BN, BN, BN, BN, BN, BN]): OrderResult {
  return {
    buyToken: result[0],
    sellToken: result[1],
    validFrom: result[2],
    validUntil: result[3],
    priceNumerator: result[4],
    priceDenominator: result[5],
    usedAmount: result[6],
  };
}
