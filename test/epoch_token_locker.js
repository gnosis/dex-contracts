...
contract("EpochTokenLocker", async (accounts) => {
  ...
  describe("all accounts", async () => {
    it("records all accounts that have ever deposited", async () => {
      const epochTokenLocker = await EpochTokenLockerTestInterface.new()
      const ERC20 = await MockContract.new()
      await ERC20.givenAnyReturnBool(true)

      await epochTokenLocker.deposit(ERC20.address, 1, { from: user_1 })
      await epochTokenLocker.deposit(ERC20.address, 2, { from: user_1 })
      await epochTokenLocker.deposit(ERC20.address, 3, { from: user_2 })

      const accounts = await epochTokenLocker.getAllAccounts()
      assert.deepEqual(accounts, [user_1, user_2])
    })
  })
})
