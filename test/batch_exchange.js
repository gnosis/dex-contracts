...
} = require("../build/common/test/utilities");
...
const {
  ...
  feeAdded,
  feeSubtracted,
} = require("../build/common/test/resources/math");
const {
...
contract("BatchExchange", async (accounts) => {
  ...
  describe("addToken()", () => {
    it("feeToken is set by default", async () => {
      ...
      assert.equal(
        (
          await batchExchange.tokenAddressToIdMap.call(feeToken.address)
        ).toNumber(),
        0,
      );
      assert.equal(
        await batchExchange.tokenIdToAddressMap.call(0),
        feeToken.address,
      );
    });
    it("Anyone can add tokens", async () => {
      ...
      assert.equal(
        (
          await batchExchange.tokenAddressToIdMap.call(token_1.address)
        ).toNumber(),
        1,
      );
      assert.equal(
        await batchExchange.tokenIdToAddressMap.call(1),
        token_1.address,
      );
      ...
      assert.equal(
        (
          await batchExchange.tokenAddressToIdMap.call(token_2.address)
        ).toNumber(),
        2,
      );
      assert.equal(
        await batchExchange.tokenIdToAddressMap.call(2),
        token_2.address,
      );
    });
    ...
    it("throws if OWL is not burned", async () => {
      ...
      assert(
        owlAmount.eq(
          await owlProxy.allowance.call(user_1, batchExchange.address),
        ),
      );
  ...
  describe("cancelOrders()", () => {
    ...
    it("frees storage of orders that are not yet valid", async () => {
      ...
      assert.equal(
        (await batchExchange.orders(user_1, 0)).priceDenominator,
        0,
  ...
  describe("replaceOrders()", () => {
    it("cancels and creates new orders", async () => {
      ...
      assert.equal(
        (await batchExchange.orders(user_1, 0)).sellToken,
        1,
        "First order should be present",
      );
      assert.equal(
        (await batchExchange.orders(user_1, 1)).sellToken,
        0,
        "Second order should be removed",
      );
      assert.equal(
        (await batchExchange.orders(user_1, 2)).sellToken,
        6,
        "Third order should be present",
      );
      assert.equal(
        (await batchExchange.orders(user_1, 3)).sellToken,
        7,
  ...
  describe("submitSolution()", () => {
    ...
    it("[Basic Trade] places two orders, first matches them partially and then fully in a 2nd solution submission", async () => {
      ...
      assert(
        (await batchExchange.getCurrentObjectiveValue.call()).eq(
          fullSolution.objectiveValue,
        ),
      );
    ...
    it("ensures half of the token imbalance (fees) is burned and that better solutions don't double-burn", async () => {
      ...
      const nextBatchId = (
        await batchExchange.getCurrentBatchId.call()
      ).toNumber();
    ...
    it("ensures that a solution reversion can not be prevented by additional withdrawRequests", async () => {
      ...
      await batchExchange.requestWithdraw(
        await batchExchange.tokenIdToAddressMap.call(
          basicTrade.orders[1].buyToken,
        ),
        1,
        {
    ...
    it("ensures credited tokens can't be withdrawn in same batch as solution submission", async () => {
      ...
      const buyToken = await batchExchange.tokenIdToAddressMap.call(
        basicTrade.orders[0].buyToken,
      );
      ...
      assert.equal(
        batchId + 1,
        (
          await batchExchange.lastCreditBatchId.call(relevantUser, buyToken)
        ).toString(),
    ...
    it("ensures credited feeToken reward can't be withdrawn in same batch as solution submission", async () => {
      ...
      assert.equal(
        batchId + 1,
        (
          await batchExchange.lastCreditBatchId.call(solver, feeToken)
        ).toString(),
      );
      await truffleAssert.reverts(
    ...
    it("checks that the objective value is returned correctly after getting into a new batch", async () => {
      ...
      assert.equal(
        0,
        await batchExchange.getCurrentObjectiveValue.call(),
    ...
    it("reverts if max touched orders is exceeded", async () => {
      ...
      const maxTouchedOrders = (
        await batchExchange.MAX_TOUCHED_ORDERS.call()
      ).toNumber();
    ...
    it("[Ring Trade] settles a ring trade between 3 tokens", async () => {
      ...
      for (let i = 0; i < basicRingTrade.orders.length; i++) {
        ...
        const sellToken = await batchExchange.tokenIdToAddressMap.call(
          order.sellToken,
        );
        const relevantUser = accounts[order.user];

        const sellTokenBalance = await batchExchange.getBalance.call(
          relevantUser,
          sellToken,
        );
        const buyTokenBalance = await batchExchange.getBalance.call(
          relevantUser,
          buyToken,
        );
    ...
    it("checks that solution trades are deleted even if balances are temporarily negative while reverting ", async () => {
      ...
      assert.equal(
        (
          await batchExchange.getBalance.call(accounts[0], otherToken)
        ).toString(),
      ...
      assert.equal(
        0,
        await batchExchange.getBalance.call(accounts[1], feeToken),
        0,
        "Sold tokens were not adjusted correctly",
      );
      // User 2
      ...
      assert.equal(
        (
          await batchExchange.getBalance.call(accounts[2], otherToken)
        ).toString(),
    ...
    it("ensures order's usedAmount is not updated, if tracking is unintended", async () => {
      ...
      const orderResult1 = await batchExchange.orders.call(
      ...
      const orderResult2 = await batchExchange.orders.call(
        firstSolution.owners[1],
        firstSolution.touchedorderIds[1],
      );
      assert.equal(
      ...
      const orderResult3 = await batchExchange.orders.call(
        secondSolution.owners[0],
        secondSolution.touchedorderIds[0],
      );
      assert.equal(
        orderResult3.usedAmount.toString(),
        0,
        "usedAmount was modified, although it should not have been modified",
      );
    });
  ...
  describe("getEncodedUserOrdersPaginated()", async () => {
    it("returns correct orders considering offset and pageSize", async () => {
      ...
      const auctionElements = decodeOrdersBN(
  ...
  describe("getEncodedOrders()", async () => {
    ...
    it("credits balance when it's valid", async () => {
      ...
      let auctionElements = decodeOrdersBN(
        await batchExchange.getEncodedOrders(),
      );
  ...
  describe("getEncodedUsersPaginated", async () => {
    ...
    it("returns three orders one per page", async () => {
      ...
      const firstPage = decodeOrdersBN(
      ...
      const secondPage = decodeOrdersBN(
      ...
      const thirdPage = decodeOrdersBN(
