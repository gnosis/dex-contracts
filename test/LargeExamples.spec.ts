import { solutionSubmissionParams, largeRing30 } from "./resources/examples";
import {
  closeAuction,
  makeDeposits,
  placeOrders,
...
contract("BatchExchange", async (accounts) => {
  ...
  describe("Large Examples [ @skip-on-coverage ]", () => {
    it("ensures hard gas limit on largest possible ring trade ", async () => {
      ...
      const solution2 = solutionSubmissionParams(
        tradeExample.solutions[1],
        accounts,
        orderIds,
      );
