...
contract("BatchExchangeViewer [ @skip-on-coverage ]", (accounts) => {
  ...
  describe("getOpenOrderBook", () => {
    ...
    it("can be queried with pagination", async () => {
      ...
      const viewer = await BatchExchangeViewer.new(batchExchange.address);
      const result = await viewer.getOpenOrderBookPaginated(
        [],
        zero_address,
        0,
        5,
      ...
      assert.equal(
        decodeIndexedOrdersBN(result.elements).filter(
          (e) => e.validFrom == batchId,
        ).length,
        5,
      );
    ...
    it("can filter a token pair", async () => {
      ...
      const viewer = await BatchExchangeViewer.new(batchExchange.address);
      const result = decodeIndexedOrdersBN(
  ...
  describe("getFinalizedOrderBook", () => {
    ...
    it("can be queried with pagination", async () => {
      ...
      const viewer = await BatchExchangeViewer.new(batchExchange.address);
      const result = await viewer.getFinalizedOrderBookPaginated(
        [],
        zero_address,
        0,
        5,
