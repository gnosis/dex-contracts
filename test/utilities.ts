...
const send = function <T>(method: string, params: T[], web3Provider: Web3): Promise<{}> {
  return new Promise(function (resolve, reject) {
    ;(web3Provider.currentProvider as HttpProvider).send({ id, jsonrpc, method, params }, (error, result) => {
      if (error) {
        reject(error)
      } else {
        resolve(result)
 ...
 */
export const waitForNSeconds = async function (seconds: number, web3Provider = web3 as Web3): Promise<void> {
  await send("evm_increaseTime", [seconds], web3Provider)
  await send("evm_mine", [], web3Provider)
}

export const closeAuction = async function (instance: EpochTokenLockerInstance, web3Provider = web3 as Web3): Promise<void> {
