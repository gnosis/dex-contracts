...
function assertOffers(
  ...
  for (let i = 0; i < offers.bids.length; i++) {
    assert.equal(
      offers.bids[i].price.toNumber(),
      bids[i][0],
      "bid price does not match",
    );
    assert.equal(
      offers.bids[i].volume.toNumber(),
      bids[i][1],
      "bid volume does not match",
    );
  }
  for (let i = 0; i < offers.asks.length; i++) {
    assert.equal(
      offers.asks[i].price.toNumber(),
      asks[i][0],
      "ask price does not match",
    );
    assert.equal(
      offers.asks[i].volume.toNumber(),
      asks[i][1],
      "ask volume does not match",
    );
  }
}
...
describe("Orderbook", () => {
  ...
  describe("fee mechanism", () => {
    ...
    it("multiplies fee when building transitive closure", () => {
      ...
      const second_orderbook = new Orderbook("DAI", "TUSD", {
        fee: new Fraction(1, 100),
      });
  ...
  describe("serialize", () => {
    it("can be deserialized", () => {
      const original = new Orderbook("USDC", "DAI", {
        fee: new Fraction(0, 1),
      });
...
describe("transitiveOrderbook", () => {
  it("computes transitive orderbook with 0 hops", () => {
    ...
    assert.equal(
      JSON.stringify(orderbook.getOffers()),
      JSON.stringify(transitive.getOffers()),
    );
  });
