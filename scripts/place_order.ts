...
const argv = require("yargs")
  ...
  .option("minBuy", {
    describe:
      "minimum amount to be bought (in 10**18 WEI of buyToken, e.g. 1 = 1ETH)",
  })
  .option("maxSell", {
    describe:
      "minimum amount to be sold (in 10**18 WEI of sellToken, e.g. 1 = 1ETH)",
  })
  .option("validFor", {
...
module.exports = async (callback: Truffle.ScriptCallback) => {
  try {
    ...
    log.info(
      `Limit Sell Order placed ID ${id}. Valid from batch ${batch_index} until ${valid_until}`,
    );
    callback();
  } catch (error) {
    callback(error);
