...
const getOrderData = async function (instance, callback, web3, argv) {
  ...
  const buyTokenAddress = await instance.tokenIdToAddressMap.call(
    argv.buyToken,
  );
  ...
  const sellTokenAddress = await instance.tokenIdToAddressMap.call(
    argv.sellToken,
  );
...
const addTokens = async function ({
  tokenAddresses,
  account,
  batchExchange,
  owl,
  ...
  const allowanceOfOWL = await owl.allowance.call(
    account,
...
async function mintToken({ token, account, minter, amount }) {
  console.log(
    "Mint %d of token %s for user %s. Using %s as the minter",
    amount,
    token.address,
    account,
    minter,
...
async function deleteOrders({ orderIds, accounts, batchExchange }) {
  ...
  for (let i = 0; i < orderIds.length; i++) {
    ...
    console.log(
      "Canceled/Deleted order %s for user %s. Emitted events: %s",
      orderId.toString(10),
      account,
      events,
    );
...
async function setAllowance({ token, account, amount, batchExchange }) {
  console.log(
    "Set allowance of %d for token %s and user %s",
    amount,
    token.address,
    account,
  );
