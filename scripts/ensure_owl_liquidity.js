...
const containsSellOrderProvidingLiquidity = function (orders) {
  return orders.some(
    (order) =>
      order.sellTokenBalance.gt(MINIMAL_LIQUIDITY_FOR_OWL) &&
      order.remainingAmount.gt(MINIMAL_LIQUIDITY_FOR_OWL),
  );
};
...
const hasOWLLiquidityOrderAlreadyBeenPlaced = function (orders) {
  return orders.some(
    (order) =>
      order.priceDenominator.eq(SELL_ORDER_AMOUNT_OWL) &&
      order.validUntil.eq(maxUint32),
  );
};
...
module.exports = async (callback) => {
  try {
    ...
    const owlBalance = await instance.getBalance(
      liquidityEnsurer,
      owlTokenAddress,
    );
    if (new BN(10).pow(new BN(18)).gt(owlBalance)) {
      callback(
        "Error: The OWL balance is below the 10 OWL threshold, please stock it up again",
      );
    }
