...
```sh
yarn texec scripts/place_order.js --accountId=0 --buyToken=1 --sellToken=0 --minBuy=1 --maxSell=2 --validFor=5 --network=rinkeby
```
...
```sh
yarn texec scripts/deposit.js --accountId=0 --tokenId=0 --amount=30 --network=rinkeby
```

### Request Withdraw

```sh
yarn texec scripts/request_withdraw.js --accountId=0 --tokenId=0 --amount=30 --network=rinkeby
```
...
```sh
yarn texec scripts/claim_withdraw.js --accountId=0 --tokenId=0 --network=rinkeby
```
...
```sh
yarn texec scripts/invoke_view_function.js --network mainnet getSecondsRemainingInBatch
```

Some other potentially useful calls might be,

```sh
yarn texec scripts/invoke_view_function.js --network mainnet getCurrentBatchId
yarn texec scripts/invoke_view_function.js --network mainnet tokenIdToAddressMap 1
```
...
```sh
yarn texec scripts/get_auction_elements.js --network rinkeby --expired true --covered false --tokens 0,1,2,3,4 --pageSize 50
```
...
```sh
yarn texec scripts/transitive_orderbook.js --network rinkeby --sellToken 1 --buyToken 4 --sellAmount 1000
```
...
```sh
yarn texec scripts/add_token_list.js
```
...
```sh
yarn texec scripts/ensure_owl_liquidity.js --network rinkeby
```
...
```sh
yarn texec scripts/place_spread_orders.js  --tokens 2,3,6 --network rinkeby
```
...
```sh
yarn texec scripts/ganache/setup_environment.js
```
...
```sh
yarn texec scripts/ganache/setup_thegraph_data.js
```
...
```sh
yarn texec scripts/ganache/wait_seconds.js  10
```

### Close Auction

```sh
yarn texec scripts/ganache/close_auction.js
```
